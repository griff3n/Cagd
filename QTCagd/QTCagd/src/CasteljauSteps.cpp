...
CasteljauSteps::CasteljauSteps(std::vector<glm::vec3> positions, glm::vec3 color)
{
	CasteljauSteps::initShader(programSimple, "shader/simple.vert", "shader/simple.frag");
	initObject(positions, color);
}

void CasteljauSteps::render(const glm::mat4 & view, const glm::mat4 & projection)
{
	// Create mvp.
...
void CasteljauSteps::translate(glm::vec3 vector) {
	model = model * glm::translate(vector);
}

void CasteljauSteps::rotateAroundAxis(float rot_speed, glm::vec3 axis, glm::vec3 positionOfAxis) {
...
void CasteljauSteps::initObject(const std::vector<glm::vec3> bPositions, const glm::vec3 & color)
{
...
void CasteljauSteps::initShader(GLSLProgram & program, const std::string & vert, const std::string & frag)
{
