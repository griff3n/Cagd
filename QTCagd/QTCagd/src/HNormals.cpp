...
HNormals::HNormals(std::vector<glm::vec3> positions, glm::vec3 color)
{
	HNormals::initShader(programSimple, "shader/simple.vert", "shader/simple.frag");
	initObject(positions, color);
}

void HNormals::render(const glm::mat4 & view, const glm::mat4 & projection)
{
	// Create mvp.
...
void HNormals::translate(glm::vec3 vector) {
	model = model * glm::translate(vector);
}

void HNormals::rotateAroundAxis(float rot_speed, glm::vec3 axis, glm::vec3 positionOfAxis) {
...
void HNormals::initObject(const std::vector<glm::vec3> hnPositions, const glm::vec3 & color)
{
	// Build the axis.
...
void HNormals::initShader(GLSLProgram & program, const std::string & vert, const std::string & frag)
{
